...
def draw(l1, l2):
	...
	index = 0;
	line_index = 0;
	linelabels = ["with mptcp", "without mptcp", "dynamic","HP,FW","FM,FW","FM,HP,FW"]
	ax = plt.subplot()
	for l in lines:
		l1 = ax.errorbar(timeline, l, fmt = styles[index], color=colors[index], label=linelabels[line_index])
	...
	ax.set_yscale('log') # if want to linearly scale the y axis, comment this line
